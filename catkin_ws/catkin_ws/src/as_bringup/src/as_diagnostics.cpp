...
#include <as_msgs/SensorState.h>
#include <as_msgs/VersionInfo.h>
#include <string>
...
void sensorStateMsgCallback(const as_msgs::SensorState::ConstPtr &msg)
{
  if (msg->battery > 11.0)
  ...
  if (msg->button == as_msgs::SensorState::BUTTON0)
    setButtonDiagnosis(diagnostic_msgs::DiagnosticStatus::OK, "BUTTON 0 IS PUSHED");
  else if (msg->button == as_msgs::SensorState::BUTTON1)
...
void firmwareVersionMsgCallback(const as_msgs::VersionInfo::ConstPtr &msg)
{
  ...
  if (check_version == false)
  {
    if (firmware_version.major_number == FIRMWARE_VERSION_MAJOR_NUMBER)
    {
      if (firmware_version.minor_number > FIRMWARE_VERSION_MINOR_NUMBER)
      {
        ...
        ROS_WARN("You can find how to update its in `FAQ` section(as.robotis.com)");
      }
    }
    else
    {
...
{
  ros::init(argc, argv, "as_diagnostic");
  ros::NodeHandle nh;

  tb3_diagnostics_pub  = nh.advertise<diagnostic_msgs::DiagnosticArray>("diagnostics", 10);
  tb3_version_info_pub = nh.advertise<as_msgs::VersionInfo>("version_info", 10);
